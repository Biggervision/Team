...
A dependency-free command line client for reading and writing a WordPress site
over the REST API. Copy it into a project and set the two site constants below.

Defaults to staging. Production is only targeted when WP_SITE says so
explicitly, and the client warns on stderr when it does — the warning exists
because "I thought I was on staging" is the expensive mistake in this work.
...
or from an untracked `.env.wordpress` file at the repo root:
    ...
    WP_SITE=https://staging.example.com
...
Some hosts answer rate-limited requests with a captcha interstitial rather than
an error. Those are detected and retried automatically: the challenge is served
at the edge and never reaches WordPress, so retrying a POST cannot double-apply
a write.
"""
...
# --- set these two per project -------------------------------------------
STAGING_SITE = "https://staging.example.com"
PRODUCTION_SITE = "https://example.com"
AGENCY = "Agency"          # used in the User-Agent below
# --------------------------------------------------------------------------

# Work happens on staging unless WP_SITE explicitly says otherwise.
DEFAULT_SITE = STAGING_SITE

# Identify the tool honestly. A spoofed browser string is what caused every
# block on the project this came from: hosting WAFs carry rules against
# outdated Chrome user agents, because that is what malicious traffic claims to
# be, and tripping them greylists the whole egress IP range.
#
# Saying what the software is and where to complain about it is both the honest
# answer and the one that works — support can allowlist a named client and
# cannot help a fake browser. Never put a browser string here.
USER_AGENT = f"{AGENCY}-ContentSync/1.0 (WordPress REST client; +{PRODUCTION_SITE})"

TIMEOUT = 60

# A genuine burst challenge clears on the next attempt, so one quick retry is
# worth having. Beyond that, retrying is the problem rather than the cure:
# repeated challenged requests are exactly what gets an IP range greylisted, so
# grinding through them extends the block and hides it from whoever is running
# the job.
CAPTCHA_RETRIES = 2
CAPTCHA_BACKOFF = [1, 3]

# If challenges keep coming across separate calls, the host is no longer having
# a bad moment — it is refusing us. Stop the whole run rather than let a loop
# quietly turn one blocked request into hundreds.
...
class HostRefusing(WPError):
    """Raised when challenges stop being transient. Do not catch and retry."""
...
def warn_if_production(site: str) -> None:
    ...
    if not _warned_production and site.rstrip("/") == PRODUCTION_SITE:
        ...
        print(
            "WARNING: targeting PRODUCTION, not staging. "
...
def request(
    ...
    """Call the WordPress REST API, retrying past the host's rate limiter.
    ...
    for attempt in range(CAPTCHA_RETRIES + 1):
        try:
            result = _request_once(method, route, params, data, body, content_type, extra_headers)
            _consecutive_challenges = 0
            return result
        except CaptchaChallenge:
            if attempt == CAPTCHA_RETRIES:
                _consecutive_challenges += 1
                if _consecutive_challenges >= CIRCUIT_BREAK_AFTER:
                    raise HostRefusing(
                        f"The host has challenged {_consecutive_challenges} requests in a row. "
                        "This is no longer a transient burst — stop the run.\n\n"
                        "Continuing renews the block rather than getting through it. Check the "
                        "User-Agent first (a browser string is the usual cause), capture the "
                        "response headers as evidence for the host, then leave the site alone "
                        "for a while before trying again."
                    ) from None
                raise WPError(
                    "The host returned a bot-protection captcha instead of the API response, "
                    f"and it did not clear after {CAPTCHA_RETRIES} retries "
                    f"({_consecutive_challenges} in a row now)."
                ) from None
            delay = CAPTCHA_BACKOFF[min(attempt, len(CAPTCHA_BACKOFF) - 1)]
            print(f"note: challenged by the host, one retry in {delay}s", file=sys.stderr)
            time.sleep(delay)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="wp.py",
        description="WordPress REST client.",
